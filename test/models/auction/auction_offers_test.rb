...
class AuctionOffersTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @deposit_english = auctions(:deposit_english)
  ...
  def test_without_result_scope_does_not_return_active_auctions
    ...
    assert_equal([@persisted_auction, @other_persisted_auction,
                  @orphaned_auction, @with_invoice_auction, @english, @deposit_english].to_set,
                 Auction.active.to_set)
  end

  def test_without_result_scope_returns_auctions_that_do_not_have_results
    travel_back
    assert_equal([@persisted_auction, @english, @deposit_english].to_set,
                 Auction.without_result.to_set)
    assert_equal([@persisted_auction, @other_persisted_auction,
                  @expired_auction, @orphaned_auction, @with_invoice_auction,
                  @english, @english_nil, @deposit_english].to_set,
                 Auction.all.to_set)
  end
end
