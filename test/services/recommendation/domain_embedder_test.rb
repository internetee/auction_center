...
module Recommendation
  class DomainEmbedderTest < ActiveSupport::TestCase
    def test_returns_aligned_embeddings_for_each_row
      rows = [
        { domain_name: 'a.ee', description: 'first', keywords: %w[one] },
        { domain_name: 'b.ee', description: 'second', keywords: %w[two] }
      ]
    ...
    def test_raises_on_openai_error
      ...
      assert_raises(StandardError) do
        Recommendation::DomainEmbedder.call(rows: [{ domain_name: 'x.ee', description: 'x', keywords: [] }])
      end
    end
