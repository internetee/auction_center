...
class InvoicesPayAllCancelledPayableInvoicesControllerTest < ActionDispatch::IntegrationTest
  ...
  def post_create
    post pay_all_cancelled_payable_invoices_path
  end
  ...
  def test_create_sets_flash_and_redirects_to_invoices_when_service_fails
    ...
    relation = Invoice.none
    response_double = Struct.new(:result?, :instance, :errors).new(false, {}, { 'message' => 'bulk failed' })
