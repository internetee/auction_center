...
class EnglishOffersIntegrationTest < ActionDispatch::IntegrationTest
  ...
  def test_when_user_create_bid_created_domain_history_snapshot
    @auction.offers.destroy_all
    @auction.reload
    assert @auction.offers.empty?
  ...
  def test_when_user_update_bid_created_domain_history_snapshot
    @auction.offers.destroy_all
    @auction.reload
    assert @auction.offers.empty?
    ...
    assert_difference -> { DomainOfferHistory.count } do
      patch english_offer_path(uuid: offer1.uuid),
          params: params,
          headers: {}
    end
  end
